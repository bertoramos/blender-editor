...
def autoregister():
    global classes
    classes = [RobotItemForSelect, RobotItemForDelete, AddRobotOperator, DeleteRobotOperator, SelectRobotProps, SelectRobotOperator, SaveRobotOperator, LoadRobotOperator]
    for cls in classes:
...
def export_robot(path, name, robot_data):
...
def read_robot(path, name, context):
    ...
    with open(path / name, 'r') as outfile:
        ...
        scene.robot_props.prop_robot_name = data['name']
        scene.robot_props.prop_robot_loc = Vector(tuple(data['location']))
        scene.robot_props.prop_robot_type = data['robot_type'][0]
        scene.robot_props.prop_ip = data['ip']
        scene.robot_props.prop_port = data['port']
        scene.robot_props.prop_client_ip = data['client_ip']
        scene.robot_props.prop_client_port = data['client_port']

        if data['robot_type'][0] == 'ROBOMAP':
            scene.myrobot_props.prop_myrobot_rotation = data['rotation'][2]
            scene.myrobot_props.prop_myrobot_dim = Vector(tuple(data['dim']))
            scene.myrobot_props.prop_myrobot_margin = Vector(tuple(data['margin']))

        bpy.ops.scene.add_robot()
        return True
    return False

class SaveRobotOperator(bpy.types.Operator, ExportHelper):
    bl_idname = "scene.save_robot_operator"
    bl_label = "Save robot"
    bl_description = "Save robot in a json file"
    ...
    def draw(self, context):
        self.layout.label(text="Select a robot to export")
    ...
    def execute(self, context):
        ...
        robot_data = robot.export()

        export_robot(path, name, robot_data)

        self.report({'INFO'}, f"{robot.name} exported")

        return {'FINISHED'}

class LoadRobotOperator(bpy.types.Operator, ImportHelper):
    bl_idname = "scene.load_robot_operator"
    bl_label = "Load robot"
    bl_description = "Load robot in a json file"
    ...
    def execute(self, context):
        ...
        if read_robot(path, name, context):
            self.report({'INFO'}, f"{self.filepath} imported")
        else:
            self.report({'ERROR'}, f"{self.filepath} cannot be imported")
