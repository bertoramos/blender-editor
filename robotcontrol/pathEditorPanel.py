...
class ToolsPanel(bpy.types.Panel):
    ...
    def draw(self, context):
        add_pose_button_text = ""
        if context.scene.isModifying:
            add_pose_button_text = "Apply changes"
        else:
            add_pose_button_text = "Append pose"

        self.layout.operator(pe.SavePoseOperator.bl_idname, icon="PLUS", text=add_pose_button_text)
